...
class Pilha{
	...
	public void inserir(Presidente presir)throws Exception{
		if(n>=array.length){
	...
	public Presidente remover(){
		Presidente presir = new Presidente();
		presir = array[n-1];
		n--;
		//imprimir();
		return presir;
...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		Pilha pilhaDePresidentes = new Pilha(50);
		String remover[] = new String[30];
		entrada = MyIO.readLine();
		int i = 0, a = 0, limitador =0;
		while(!entrada.equals("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			pilhaDePresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}

		entrada = MyIO.readLine();
		limitador = Integer.parseInt(entrada);
		String local = "";
		int n=0;
		...
		for(int x=0;x<limitador;x++){
			...
			if(entrada.charAt(0)=='I'){
				local = entrada.substring(2,entrada.length());
				pres.ler(local);
	                        pilhaDePresidentes.inserir(pres);
			}
			if(entrada.charAt(0)=='R'){
                                pres = pilhaDePresidentes.remover();
				remover[n]=pres.getNome();
				n++;
                        }
		}
		...
		MyIO.setCharset("ISO-8859-1");
		for(int x=0;x<n;x++){
			MyIO.println("(R) "+remover[x]);
		}
		/*for(i=0;i<pilhaDePresidentes.length;i++){
			System.out.print("["+i+"]")
			pilhaDePresidentes.imprimir();
