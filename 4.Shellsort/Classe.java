...
class Shell{
	...
	public void insercaoPorCor(int cor, int h){
		for (int i = (h + cor); i < n; i+=h) {
			Presidente tmp = new Presidente();
			tmp = array[i];
			int j = i - h;
			while ((j >= 0) && (array[j].getPagina().compareTo(tmp.getPagina())>0)) {
				array[j + h] = array[j];
				j-=h;
				movimentacoes++;
				comparacoes++;
...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		Shell ShellSortPresidentes = new Shell(60);
		entrada = MyIO.readLine();
		while(!entrada.equals("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			ShellSortPresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}
		ShellSortPresidentes.shellsort();
		long fim= now();
		long tempo = fim-comeco;


		MyIO.setCharset("ISO-8859-1");
		for(int x=0; x<ShellSortPresidentes.getN();x++){
			ShellSortPresidentes.imprimir(x);
		}

		//criar arquivo com esse nome:
		File file = new File("matrícula_shellsort.txt");
		...
		arquivo="631825"+"\t"+ShellSortPresidentes.getComparacoes()+"\t"+ShellSortPresidentes.getMovimentacoes()+"\t"+tempo+"\t";
		bw.write(arquivo);
		bw.close();
