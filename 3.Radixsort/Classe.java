...
class Radix{
	...
	public int getMax() 
	{ 
		Presidente mx = new Presidente();
		mx = array[0]; 
		for (int i = 1; i < n; i++){ 
			if (array[i].getId() > mx.getId()){
				mx = array[i];
				comparacoes++;
			}
		}	
	...
	{ 
		...
		int i; 
		int count[] = new int[getMax()]; 
		Arrays.fill(count,0); 

		// Store count of occurrences in count[] 
		for (i = 0; i < n; i++) 
			count[ (array[i].getId()/exp)%10 ]++; 
		...
		for (i = n - 1; i >= 0; i--) 
		{ 
			output[count[ (array[i].getId()/exp)%10 ] - 1] = array[i]; 
			count[ (array[i].getId()/exp)%10 ]--; 
			movimentacoes++;
		} 
...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		Radix radixPresidentes = new Radix(60);
		entrada = MyIO.readLine();
		while(!entrada.equals("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			radixPresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}
		radixPresidentes.radixsort();
		long fim= now();
		long tempo = fim-comeco;


		MyIO.setCharset("ISO-8859-1");
		for(int x=0;x<radixPresidentes.getN();x++){
			radixPresidentes.imprimir(x);
		}

		//criar arquivo com esse nome:
		File file = new File("matrícula_radixsort.txt");
		...
		arquivo="631825"+"\t"+radixPresidentes.getComparacoes()+"\t"+radixPresidentes.getMovimentacoes()+"\t"+tempo+"\t";
		bw.write(arquivo);
		bw.close();
