...
class ArvoreTrie {
	...
	public void mostrar(String palavra, No no) {
		if(no.folha == true){
			palavra = "" + palavra + no.elemento;
			MyIO.println("palavra atual: "+palavra);
		} else {
	...
	public void merge (ArvoreTrie t1){
		merge("", t1.getRaiz());
	}
	public void merge(String palavra, No no){
		if(no.folha == true){
			String nova = palavra + no.elemento;
			inserir(nova.substring(1));
		}else{
			for(int i=0;i<no.prox.length;i++){
				if(no.prox[i] != null){
					merge(palavra+no.elemento,no.prox[i]);
				}
			}
		}
	}
...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		while(!entrada.contains("FIM")){
			...
			if(Trie3.pesquisar(entrada)==true){
				outISO.print(" SIM\n");
			}else{
				outISO.print(" NÃO\n");
			}
