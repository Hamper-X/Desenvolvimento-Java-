...
class Hash {
	...
	public Hash (int tamanho){
		...
		this.comparacoes =0;
		this.tabela = new Presidente [this.tamanho];
	...
	public int reHash(Presidente elemento){
		int resp=-1;
		if(controle<reserva){
			resp = controle;
			controle++;
	...
	public boolean pesquisar (Presidente elemento){
		...
		if(tabela[pos] == elemento){
			...
			for(int i=tamanho-reserva;i<tamanho;i++){
				if(tabela[pos]==elemento){
					resp = true;
					i = tamanho;
				}
	...
	public boolean pesquisarString(String nome){
		boolean resp = false;
		for(int i=0;i<tamanho;i++){
			if(tabela[i]!=null){
			...
			if(tabela[pos]==elemento){
				...
				if(pos != -1){
					if(tabela[pos]==elemento){
						...
						//elemento nao esta na hash
					}
				}
			}
		}else{
			//elemento não esta na Hash
		}
		return resp;
	}
}
