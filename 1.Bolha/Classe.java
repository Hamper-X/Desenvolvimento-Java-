...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		Bolha bolhaPresidentes = new Bolha(60);
		entrada = MyIO.readLine();
		while(!entrada.equals("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			bolhaPresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}
		bolhaPresidentes.bolha();
		long fim= now();
		long tempo = fim-comeco;


		MyIO.setCharset("ISO-8859-1");
		for(int x=0;x<bolhaPresidentes.getN();x++){
			bolhaPresidentes.imprimir(x);
		}

		//criar arquivo com esse nome:
		File file = new File("matrícula_bolha.txt");
		...
		arquivo="631825"+"\t"+bolhaPresidentes.getComparacoes()+"\t"+bolhaPresidentes.getMovimentacoes()+"\t"+tempo+"\t";
		bw.write(arquivo);
		bw.close();
