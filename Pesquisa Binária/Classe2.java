...
class Selecao{
	...
	public boolean PesquisaBinaria(String nome){
		...
		while(esq<=dir){
			meio = (esq + dir)/2;
			if(array[meio].getNome().compareTo(nome)==0){
				resp = true;
				esq = dir+1;
			}else{
				if(array[meio].getNome().compareTo(nome)<0){
...
public class Classe2{
	...
	public static void main(String[] args)throws Exception{
		...
		String arquivo;
		Selecao selecaoPresidentes = new Selecao(60);
		entrada = MyIO.readLine();
		while(!entrada.contains("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			selecaoPresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}
		selecaoPresidentes.ordenar();
		
		PrintStream out = new PrintStream(System.out, true, "ISO-8859-1");
		entrada=MyIO.readLine();
		while(!entrada.contains("FIM")){
			if(selecaoPresidentes.PesquisaBinaria(entrada)==true){
				out.println("SIM");
			}else{
				out.println("NÃO");
			}
		...
		long tempo = fim-comeco;
                ...
                contador = selecaoPresidentes.getContador();


                        //criar arquivo com esse nome:
                        File file = new File("matrícula binaria.txt");
