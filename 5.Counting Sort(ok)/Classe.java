...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		Coutin coutinPresidentes = new Coutin(60);
		entrada = MyIO.readLine();
		while(!entrada.equals("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			coutinPresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}
		coutinPresidentes.countinOrdenado();
		long fim= now();
		long tempo = fim-comeco;


		MyIO.setCharset("ISO-8859-1");
		for(int x=0;x<coutinPresidentes.getN();x++){
			coutinPresidentes.imprimir(x);
		}
	}
}
