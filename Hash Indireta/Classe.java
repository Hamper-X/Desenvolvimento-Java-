...
class Celula {
	...
	Celula(Presidente elemento, Celula prox) {
		this.elemento = elemento;
		this.prox = prox;
	}
}
...
class Lista {
	...
	public boolean pesquisarString(String x) {
                boolean retorno = false;
                for (Celula i = primeiro.prox; i != null; i = i.prox) {
	...
	public void inserirFim(Presidente elemento) {
		Celula tmp = new Celula(elemento);
		ultimo.prox = tmp;
		ultimo = ultimo.prox;
		tmp = null;
	...
	public Presidente removerInicio() throws Exception {
		Presidente resp = new Presidente();
...
class HashIndiretoLista {
	...
	public int getHash(Presidente elemento){
		return (elemento.getId() % tamanho);
	}

	public boolean pesquisar(Presidente elemento){
		int pos = getHash(elemento);
		return tabela[pos].pesquisar(elemento);
	}
	public boolean pesquisarString(String elemento){
                boolean resp = false;
		for(int i=0;i< tamanho;i++){
			if(tabela[i].pesquisarString(elemento)==true){
				i=tamanho;
				resp = true;
...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		HashIndiretoLista HashVetor = new HashIndiretoLista(21);
		...
		while(!entrada.contains("FIM")){
			...
			pres.ler(entrada);
			HashVetor.inserirInicio(pres);
			entrada = MyIO.readLine();
		}

		entrada = MyIO.readLine();
		PrintStream outISO = new PrintStream(System.out, true, "ISO-8859-1");
		PrintStream outUTF = new PrintStream(System.out, true, "UTF-8");
		while(!entrada.contains("FIM")){
			outUTF.print(entrada);
			if(HashVetor.pesquisarString(entrada)==true){
				outISO.print(" SIM\n");
			}else{
				outISO.print(" NÃO\n");
			}
		...
		File file = new File("matrícula_hashRehash.txt");
		// Se o arquivo nao existir, ele gera
		if (!file.exists()) {
			file.createNewFile();
		}
		long tempo = now()-comeco;
		...
		arquivo="631825"+"\t"+"\t"+tempo+"\t"+HashVetor.getComparacoes()+"\t";
		bw.write(arquivo);
		bw.close();
	}
}
