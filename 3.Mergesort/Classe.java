...
class Merge{
	...
	/*
	public int converterGetData(Presidente []L,int p1,Presidente []R,int p2){
		//1=primeiro maior que segundo 2=segundo maior que primeiro 0=iguais
		String nova1 = "";
		String nova2 = "";
		String reserva1="",reserva2="";
		int []clear1=new int[2];
		int []clear2=new int[2];
		int resp;

		nova1=""+L[p1].getFimMandato();
		nova2=""+R[p2].getFimMandato();

		for(int x=0;x<nova1.length();x++){
			if(nova1.charAt(x)>='0' && nova1.charAt(x)<='9'){
				reserva1 = reserva1 + nova1.charAt(x);
			}
			if(nova2.charAt(x)>='0' && nova2.charAt(x)<='9'){
                                reserva2 = reserva2 + nova2.charAt(x);
                        }
		}

		clear1[0]=Integer.parseInt(reserva1.substring(0,1));
		clear2[0]=Integer.parseInt(reserva2.substring(0,1));

		clear1[1]=Integer.parseInt(reserva1.substring(2,3));
		clear2[1]=Integer.parseInt(reserva2.substring(2,3));


		clear1[2]=Integer.parseInt(reserva1.substring(4,7));
		clear2[2]=Integer.parseInt(reserva2.substring(4,7));


		if(clear1[2]>clear2[2]){
			resp = 1;
		}else{
			if(clear1[2]<clear2[2]){
				resp = 2;
	...
	public void merge(int l, int m, int r)
	{
		...
		while (i < n1 && j < n2)
		{
			...
			if(L[i].getFimMandato().isBefore(R[j].getFimMandato()))
			{
				array[k] = L[i];
				i++;
				movimentacoes++;
...
public class Classe{
	...
	public static void main(String[] args)throws Exception{
		...
		Merge mergePresidentes= new Merge(60);
		entrada = MyIO.readLine();

		while(!entrada.equals("FIM")){
			Presidente pres = new Presidente();
			pres.ler(entrada);
			mergePresidentes.inserir(pres);
			entrada = MyIO.readLine();
		}
		mergePresidentes.sort(0,mergePresidentes.getN()-1);
		long fim= now();
		long tempo = fim-comeco;


		MyIO.setCharset("ISO-8859-1");
		for(int x=0;x<mergePresidentes.getN();x++){
			mergePresidentes.imprimir(x);
		}

		//criar arquivo com esse nome:
		File file = new File("matrícula_mergesort.txt");
		...
		arquivo="631825"+"\t"+mergePresidentes.getComparacoes()+"\t"+mergePresidentes.getMovimentacoes()+"\t"+tempo+"\t";
		bw.write(arquivo);
		bw.close();
